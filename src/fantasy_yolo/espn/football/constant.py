POSITION_MAP = {
    ...
    'K': 17,
    'FLEX': 23,
    # fantasy-yolo: the label->id half of this map omitted BE, IR and RB/WR/TE,
    # so POSITION_MAP[POSITION_MAP[20]] raised. Player.lineupSlot is produced
    # from the id->label half, so converting a slot label back to an id — which
    # every lineup write must do — silently yielded None for exactly the bench,
    # IR and flex slots a lineup change moves players between. Appended here
    # rather than inserted: position_slot_counts slices
    # list(POSITION_MAP.values())[:n], so order of the int-keyed entries matters.
    'BE': 20,
    'IR': 21,
    'RB/WR/TE': 23,
    'DT': 8,
