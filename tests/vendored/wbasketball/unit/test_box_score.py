...
from fantasy_yolo.espn.wbasketball.box_score import BoxScore


def _make_box_score_data(home_team_id=1, away_team_id=2, winner='UNDECIDED',
                         home_score=100.0, away_score=95.0, by_matchup=True,
                         home_projected=105.0, away_projected=98.0, include_away=True):
...
class BoxScoreTest(TestCase):
    ...
    def test_box_score_projected_scores_by_matchup(self):
        ...
        data = _make_box_score_data(home_projected=105.5, away_projected=98.2, by_matchup=True)
