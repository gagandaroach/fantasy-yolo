...
from fantasy_yolo.espn.basketball.team import Team
from fantasy_yolo.espn.basketball.player import Player


def _make_team_data(team_id=1, abbrev='LAL', name='Lakers', location='Los Angeles', nickname='Lakers',
                    division_id=1, wins=10, losses=5, ties=0, points_for=1000.0, points_against=950.0,
                    playoff_seed=1, rank_final=None, acquisitions=5, drops=3, trades=1,
                    acquisition_budget_spent=50, logo_url=None, stats_data=None):
...
class TeamTest(TestCase):
    ...
    def test_team_record_stats(self):
        ...
        data = _make_team_data(wins=20, losses=10, ties=0, points_for=2000.0, points_against=1800.0)
    ...
    def test_team_transaction_stats(self):
        ...
        data = _make_team_data(acquisitions=10, drops=5, trades=2, acquisition_budget_spent=100)
    ...
    def test_team_pro_schedule_kwarg(self):
        ...
        data = _make_team_data()
        pro_schedule = {1: {'1': [{'awayProTeamId': 1, 'homeProTeamId': 5, 'date': 1700000000000}]}}
        
        team = Team(data, self.roster_data, self.schedule_data, 2023, pro_schedule=pro_schedule)
